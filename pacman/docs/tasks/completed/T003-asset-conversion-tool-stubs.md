...
- `vanguard8_port/tests/evidence/T003-tool-stubs/build_log.txt` — full
  ...
  the assembler succeeding. Current ROM size: `16384` bytes.
- `vanguard8_port/tests/evidence/T003-tool-stubs/assets_listing.txt` —
  ...
  exists.
- `vanguard8_port/tests/evidence/T003-tool-stubs/t002_rerun.txt` — T002
  regression rerun log. Current frame-60 SHA-256:
  `0421e581a0ed677c60406cfb9884571cd904bfe94db13acb26ace122c92fadcb`.
- `vanguard8_port/tests/evidence/T003-tool-stubs/t002_frame_060.ppm` —
  frame dump generated by the T002 rerun command for visual spot-checking.
...
- 2026-04-10 — activated. Reviewed `docs/VANGUARD8_PORT_PLAN.md` §3/§3.6,
  the task workflow rules, and the current `vanguard8_port/` scaffold before
  wiring the conversion stubs into the build.
- 2026-04-10 — added `tools/_common.py`, the five standalone `conv_*.py`
  stubs, `src/data.asm`, and `pack_rom.py` converter orchestration in the
  required order. The build now produces all placeholder asset files and
  still assembles a 16384-byte ROM.
- 2026-04-10 — captured acceptance evidence under
  `vanguard8_port/tests/evidence/T003-tool-stubs/`. The clean build log shows
  all converters firing in order. `assets_listing.txt` shows all 8 expected
  asset files. The T002 rerun hash matches the recorded reference hash
  `0421e581a0ed677c60406cfb9884571cd904bfe94db13acb26ace122c92fadcb`.
