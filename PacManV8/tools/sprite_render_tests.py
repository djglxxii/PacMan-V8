...
def generated_include_matches(sat: list[int], colors: list[int]) -> bool:
    ...
    expected_lines.extend(review.store_lines(review.SPRITE_SAT_SHADOW, sat))
    expected_lines.extend(["", "; Sprite Mode 2 per-slot color shadow at 0x8330."])
    expected_lines.extend(review.store_lines(review.SPRITE_COLOR_SHADOW, colors))
...
def write_vectors(
    ...
    lines.extend(
        [
            "",
            "Determinism:",
            "- animation source: fixed T013 review state, frame counter initialized to 0",
            "- coordinate source: fixed review positions, not final T015 transform output",
        ]
    )
...
def main() -> int:
    ...
    sat, colors = review.build_review_shadow(source_colors)
    ...
    write_vectors(args.vectors_output, sat, colors, frame_hash, frame_dimensions)
